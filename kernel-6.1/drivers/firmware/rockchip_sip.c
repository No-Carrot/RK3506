...
struct dram_addrmap_info *sip_smc_get_dram_map(void)
{
	...
	/*
	 * RK3506 OP-TEE does not implement SHARE_PAGE_TYPE_DDR_ADDRMAP
	 * (type 6). Keep the caller's built-in address-map defaults instead
	 * of issuing an SMC that only produces a secure-world error.
	 */

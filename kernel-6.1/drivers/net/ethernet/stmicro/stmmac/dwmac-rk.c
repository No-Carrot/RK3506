...
static int rk_gmac_clk_init(struct plat_stmmacenet_data *plat)
{
	...
	if (IS_ERR(bsp_priv->mac_clk_rx))
		dev_dbg(dev, "cannot get clock %s\n",
			"mac_clk_rx");
...
static void rk_get_eth_addr(void *priv, unsigned char *addr)
{
	...
	if (ret <= 0 ||
		...
		dev_dbg(dev, "%s: rk_vendor_read eth mac address failed (%d)\n",
			__func__, ret);
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
		if (system_serial_high || system_serial_low) {
			addr[0] = ((system_serial_high >> 24) & 0xfc) | 0x02;
			addr[1] = (system_serial_high >> 16) & 0xff;
			addr[2] = (system_serial_high >> 8) & 0xff;
			addr[3] = system_serial_high & 0xff;
			addr[4] = (system_serial_low >> 8) & 0xff;
			addr[5] = (system_serial_low + id) & 0xff;
			dev_info(dev, "%s: generate fixed eth mac address from serial: %pM\n",
				 __func__, addr);
		} else
#endif
		{
			...
			addr[4] = 0x06;
			addr[5] = id & 0xff;
			dev_warn(dev, "%s: system serial is empty, use fixed fallback mac address: %pM\n",
				 __func__, addr);
		}
	} else {

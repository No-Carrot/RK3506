...
#define HYM8563_I2C_RETRIES	3
#define HYM8563_I2C_RETRY_DELAY_US	20000
...
static int hym8563_read_byte_retry(struct i2c_client *client, u8 reg)
{
	int ret;
	int i;

	for (i = 0; i < HYM8563_I2C_RETRIES; i++) {
		ret = i2c_smbus_read_byte_data(client, reg);
		if (ret >= 0)
			return ret;

		if (i + 1 < HYM8563_I2C_RETRIES)
			usleep_range(HYM8563_I2C_RETRY_DELAY_US,
				     HYM8563_I2C_RETRY_DELAY_US + 5000);
	}
...
static int hym8563_init_device(struct i2c_client *client)
{
	...
	ret = hym8563_read_byte_retry(client, HYM8563_CTL1);
	if (ret < 0)
		return ret;
	...
	ret = hym8563_read_byte_retry(client, HYM8563_CTL2);
	if (ret < 0)
		return ret;
...
static int hym8563_resume(struct device *dev)
{
	...
	ret = hym8563_read_byte_retry(client, HYM8563_CTL1);
	if (ret < 0)
		dev_warn(&client->dev, "resume CTL1 read failed: %d\n", ret);
...
static int hym8563_probe(struct i2c_client *client)
{
	...
	ret = hym8563_read_byte_retry(client, HYM8563_SEC);
	if (ret < 0)
		return ret;

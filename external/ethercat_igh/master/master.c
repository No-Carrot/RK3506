 ...
 * \retval <0 Error code.
 */
static unsigned int ec_master_thread_rt_prio(const char *name)
{
    if (!strcmp(name, "EtherCAT-IDLE"))
        return ec_master_idle_thread_rt_prio;
    if (!strcmp(name, "EtherCAT-OP"))
        return ec_master_op_thread_rt_prio;
    return 0;
}

/*****************************************************************************/

static void ec_master_configure_thread(
        ec_master_t *master,
        const char *name
        )
{
    ...
    if (ec_master_thread_cpu >= 0) {
        if (ec_master_thread_cpu < nr_cpu_ids &&
                cpu_online(ec_master_thread_cpu)) {
            kthread_bind(master->thread, ec_master_thread_cpu);
            EC_MASTER_INFO(master, "%s thread bound to CPU%d.\n",
                    name, ec_master_thread_cpu);
        } else {
            EC_MASTER_WARN(master, "Ignoring invalid %s thread CPU%d.\n",
                    name, ec_master_thread_cpu);
        }
    }

    rt_prio = ec_master_thread_rt_prio(name);
    if (rt_prio) {
        ...
        ret = sched_setattr_nocheck(master->thread, &attr);
        if (ret) {
            EC_MASTER_WARN(master,
                    "Failed to set %s thread SCHED_FIFO priority %u (%d).\n",
                    name, rt_prio, ret);
        } else {
            EC_MASTER_INFO(master, "%s thread SCHED_FIFO priority %u.\n",
                    name, rt_prio);
        }
    }
}
        ...
        )
{
    ...
    master->thread = kthread_create(thread_func, master, "%s", name);
    ...
    ec_master_configure_thread(master, name);
    wake_up_process(master->thread);
...
{
    ec_master_t *master = (ec_master_t *) priv_data;
    unsigned int blocked_config_cycles = 0;
    unsigned int injection_wait_cycles = 0;
    unsigned int ready_config_cycles = 0;
    ...
    while (!kthread_should_stop()) {
        ...
        if (master->config_changed &&
            ...
            blocked_config_cycles++;
            if (blocked_config_cycles == 2000 ||
                    blocked_config_cycles % 10000 == 0) {
                EC_MASTER_INFO(master,
                        "servo-op-fsm blocked config_changed=%u "
                        "datagram_state=%u fsm_idle=%d "
                        "injection_rt=%u injection_fsm=%u state=%ps.\n",
                        master->config_changed,
            ...
            blocked_config_cycles = 0;
        }

        if (master->injection_seq_rt == master->injection_seq_fsm) {
            injection_wait_cycles = 0;
            if (master->config_changed &&
                    master->fsm_datagram.state != EC_DATAGRAM_SENT &&
                    master->fsm_datagram.state != EC_DATAGRAM_QUEUED) {
                ready_config_cycles++;
                if (ready_config_cycles == 2000 ||
                        ready_config_cycles % 10000 == 0) {
                    EC_MASTER_INFO(master,
                            "servo-op-fsm ready config_changed=%u "
                            "datagram_state=%u fsm_idle=%d "
                            "injection_rt=%u injection_fsm=%u state=%ps.\n",
                            master->config_changed,
                            master->fsm_datagram.state,
            ...
            ready_config_cycles = 0;
            injection_wait_cycles++;
            if (injection_wait_cycles == 2000 ||
                    injection_wait_cycles % 10000 == 0) {
                EC_MASTER_INFO(master,
                        "servo-op-fsm waiting injection "
                        "datagram_state=%u fsm_idle=%d "
                        "injection_rt=%u injection_fsm=%u "
                        "config_changed=%u state=%ps.\n",
                        master->fsm_datagram.state,
                        master->fsm.idle,
                        master->injection_seq_rt,
                        master->injection_seq_fsm,
        ...
        )
{
    ...
    EC_MASTER_INFO(master, "servo-op-request active=%u slave_count=%u.\n",
            master->active, master->slave_count);
    ...
    for (i = 0; i < master->slave_count; i++) {
        ...
        EC_MASTER_INFO(master,
                "servo-op-request slave%u current=0x%02x requested=0x%02x "
                "config=%px error=%u force=%u.\n",
                i, slave->current_state, slave->requested_state,
                slave->config, slave->error_flag, slave->force_config);
        if (slave->config) {
            ec_slave_request_state(slave, EC_SLAVE_STATE_OP);
            requested++;
        }
    }
    EC_MASTER_INFO(master, "servo-op-request requested=%u.\n", requested);
...
int ecrt_master_activate(ec_master_t *master)
{
    ...
    master->injection_seq_fsm = 0;
    master->injection_seq_rt = 0;
    ec_datagram_unqueue(&master->fsm_datagram);
    master->fsm_datagram.state = EC_DATAGRAM_INIT;
    ec_fsm_master_wakeup(&master->fsm);
    ...
    /* Let the OP thread see the active/apply-configuration state from its
     * first FSM cycle. */
    master->allow_scan = 1;
    master->active = 1;
    ...
    master->dc_offset_valid = 0;
    EC_MASTER_INFO(master,
            "servo-op-activate fsm wakeup datagram_state=%u "
            "slave_count=%u configs_empty=%u active=%u config_changed=%u.\n",
            master->fsm_datagram.state, master->slave_count,
            list_empty(&master->configs), master->active,
            master->config_changed);
...
void ecrt_master_reset(ec_master_t *master)
{
    ec_slave_config_t *sc;
    ec_slave_t *slave;
    unsigned int configs = 0, attached = 0, requested = 0, slave_configs = 0;
    unsigned int i;
    unsigned int datagram_state_before = master->fsm_datagram.state;
    int fsm_idle_before = master->fsm.idle;

    list_for_each_entry(sc, &master->configs, list) {
        configs++;
        if (!sc->slave) {
            int ret = ec_slave_config_attach(sc);
            EC_MASTER_INFO(master,
                    "servo-op-reset config %u:%u attach ret=%d "
                    "vendor=0x%08x product=0x%08x.\n",
                    sc->alias, sc->position, ret,
                    sc->vendor_id, sc->product_code);
        }
        if (sc->slave) {
            attached++;
            EC_MASTER_INFO(master,
                    "servo-op-reset config %u:%u slave%u "
                    "current=0x%02x requested=0x%02x -> OP "
                    "vendor=0x%08x product=0x%08x.\n",
                    sc->alias, sc->position, sc->slave->ring_position,
                    sc->slave->current_state, sc->slave->requested_state,
                    sc->slave->sii.vendor_id, sc->slave->sii.product_code);
    ...
    EC_MASTER_INFO(master,
            "servo-op-reset summary active=%u phase=%u slave_count=%u "
            "configs=%u attached=%u slave_configs=%u requested=%u "
            "config_changed=%u app_time=%llu dc_ref_time=%llu "
            "fsm_datagram=%u->%u fsm_idle=%d->%d "
            "injection_rt=%u injection_fsm=%u.\n",
            master->active, master->phase, master->slave_count,
            configs, attached, slave_configs, requested,
            master->config_changed,
            (unsigned long long)master->app_time,
            (unsigned long long)master->dc_ref_time,
            datagram_state_before, master->fsm_datagram.state,
            fsm_idle_before, master->fsm.idle,
            master->injection_seq_rt, master->injection_seq_fsm);
}

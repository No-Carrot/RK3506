...
/** Wake up the master state machine without discarding known bus topology.
 */
void ec_fsm_master_wakeup(
        ec_fsm_master_t *fsm /**< Master state machine. */
        )
{
    fsm->state = ec_fsm_master_state_start;
    ...
    if (master->slave_count) {
        ...
        if (master->config_changed) {
            ...
            EC_MASTER_INFO(master,
                    "servo-op-fsm configuration changed from start; "
                    "slave_count=%u active=%u.\n",
                    master->slave_count, master->active);
        ...
        )
{
    ...
    if (master->config_changed) {
        ...
        EC_MASTER_INFO(master,
                "servo-op-fsm configuration changed during state check; "
                "aborting at slave%u.\n",
                slave->ring_position);
    ...
    if ((slave->current_state != slave->requested_state
        ...
        EC_SLAVE_INFO(slave,
                "servo-op-fsm configure current=0x%02x requested=0x%02x "
                "force=%u config=%px error=%u.\n",
                slave->current_state, slave->requested_state,
                slave->force_config, slave->config, slave->error_flag);

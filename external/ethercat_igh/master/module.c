...
int ec_master_thread_cpu = -1; /**< CPU for IDLE/OP master threads. */
unsigned int ec_master_idle_thread_rt_prio; /**< SCHED_FIFO prio for IDLE. */
unsigned int ec_master_op_thread_rt_prio; /**< SCHED_FIFO prio for OP. */
...
module_param_named(thread_cpu, ec_master_thread_cpu, int, S_IRUGO);
MODULE_PARM_DESC(thread_cpu, "CPU for EtherCAT-IDLE/EtherCAT-OP threads, -1 disables binding");
module_param_named(idle_thread_rt_prio, ec_master_idle_thread_rt_prio, uint, S_IRUGO);
MODULE_PARM_DESC(idle_thread_rt_prio, "SCHED_FIFO priority for EtherCAT-IDLE, 0 keeps normal scheduling");
module_param_named(op_thread_rt_prio, ec_master_op_thread_rt_prio, uint, S_IRUGO);
MODULE_PARM_DESC(op_thread_rt_prio, "SCHED_FIFO priority for EtherCAT-OP, 0 keeps normal scheduling");
    ...
    for (i = 0; i < master_count; i++) {
        ...
        for (dev_idx = EC_DEVICE_MAIN;
            ...
            if (!master->devices[dev_idx].dev
                ...
                ret = ec_device_attach(&master->devices[dev_idx],
                        net_dev, poll, module);
                if (ret) {
                    EC_MASTER_ERR(master, "Failed to attach device %s: %d\n",
                            str, ret);

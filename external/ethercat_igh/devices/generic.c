...
static int ec_gen_device_link_ok(ec_gen_device_t *dev)
{
    return dev->used_netdev &&
        netif_running(dev->used_netdev) &&
        netif_carrier_ok(dev->used_netdev);
}

/*****************************************************************************/

static void ec_gen_device_update_link(ec_gen_device_t *dev)
{
    ecdev_set_link(dev->ecdev, ec_gen_device_link_ok(dev));
}
